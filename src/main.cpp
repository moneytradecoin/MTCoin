...
uint256 hashGenesisBlock("0x00000786ead35a886a1b83fb70515dee29101b747cafd6f0f9b626438533ef5e");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MoneyTradeCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MoneyTradeCoin Signed Message:\n";
...
{
    // MoneyTradeCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MoneyTradeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, MoneyTradeCoin - DarkGravity, written by Evan Duffield - evan@MoneyTradeCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MoneyTradeCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MoneyTradeCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "MoneyTrade";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x04ad7383b80a3c8d5066d4f75efb9dc60b4ead6bbe4ecdd4f4985923373a38a6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // MoneyTradeCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MoneyTradeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MoneyTradeCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MoneyTradeCoinMiner(CWallet *pwallet)
{
    printf("MoneyTradeCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MoneyTradeCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running MoneyTradeCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MoneyTradeCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MoneyTradeCoinMiner, pwallet));
}
